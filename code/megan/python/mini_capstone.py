...
def random_notes(list):
    '''This function will choose a random element from the a list and return it'''
...
def random_key_signature(note):
    '''This function will return the key signature of any key'''
    return keys.get_key_signature(note)

def relative_keys(key):
    '''This function will take a key and return the relative minor key'''
    return intervals.keys.relative_minor(key)

def tonic_scale(list):
    '''This function will return the notes of the tonic scale'''
    return scales.get_notes(list)
...
while True:
    ...
    if choice == 'Quit':
        ...
        while True: 
            ...
            ask = enterbox(f"How many sharps or flats are in the key of {signature}? \nEnter a positive number for sharp keys and a negative number for flat keys \n(or 'I don't know' to see answer and 'done' to exit) ")
        ...
        while True:
            ...
            if tonic_notes == 'done':
                ...
                msgbox(f"The answer was {tonic_scale(pick_a_chord)}")
            ...
            if tonic_notes == tonic_scale(pick_a_chord):
                msgbox("You got it!")
