...
def quicksort_recursion(items_list, low, high):
    if low < high:
        partition_return = partition(items_list, low, high)
        quicksort_recursion(items_list, low, partition_return)
        quicksort_recursion(items_list, partition_return + 1, high)

def partition(items_list, low, high):
    pivot = items_list[low + (high-low)//2]
    i = low - 1 
    j = high + 1
    while True:
